...
<%--向左翻页--%>                             <span aria-hidden="true">«</span></a>
                                    </li>
...
<%--向右翻页--%>                             <span aria-hidden="true">»</span></a>
                                    </li>
        ...
        if(cks.length == 0){
            ...
            if(confirm("您确定要删除"+cks.length+"条商品吗?")){
               // alert("可以进行删除!");
                ...
                //发送ajax请求,进行批量删除的提交
                $.ajax({
                    url:"${pageContext.request.contextPath}/prod/deleteMultById.action",
                    data:{"pids":pids},
                    type:"post",
                    dataType:"text",
                    success:function (msg) {
                        ...
                        //将页面上显示商品数据的容器重新加载
                        $("#table").load("http://localhost:8080/01_war_exploded/admin/product.jsp #table");
                    }
                });
            }
        }
...
/*    function one(pid,page) {
        ...
        location.href = "${pageContext.request.contextPath}/prod/update.action" + str;
    }*/

    function one(pid) {

        location.href = "${pageContext.request.contextPath}/prod/update.action?pid="+pid;
    }
</script>
<!--分页的AJAX实现-->
<script type="text/javascript">
